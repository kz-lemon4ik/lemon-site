...
export default function Logo({ isScrolled = false, isMobile = false, onMobileInteraction, className, }) {
    ...
    const handleMobileTouch = (active) => {
    ...
    return (_jsx(Link, { to: "/", className: clsx("flex items-center", !isMobile && "mr-2 sm:mr-4 hover:scale-110", !isMobile && !isMotionDisabled && "transition-transform duration-200 ease-in-out", className), "aria-label": "Go to homepage", ...linkProps, children: _jsx("img", { src: logoSrc, onError: handleError, alt: "Lost Scores Site Logo", className: clsx("w-auto", !isMotionDisabled && !isMobile && "transition-all duration-300 ease-in-out", sizeClasses) }) }));
}
