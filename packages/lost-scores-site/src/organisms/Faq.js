...
import { Calculator, ShieldCheck, Wrench } from "lucide-react";
...
const technicalFaqData = [
    ...
    {
        question: "What should I do if the program doesn't start, gives an error, or can't find my osu! folder?",
        answer: [
...
const CategoryHeader = ({ icon: Icon, children, }) => {
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true, amount: 0.3 });
    const { isMotionDisabled } = useSettings();
    if (isMotionDisabled) {
        return (_jsxs("h2", { ref: ref, className: "flex items-center justify-center sm:justify-start gap-x-3 text-2xl sm:text-3xl font-semibold font-lostheading text-white theme-is-light:text-themeLight-headingText", children: [_jsx(Icon, { className: "w-7 h-7 text-purple-400 theme-is-light:text-themeLight-primary", strokeWidth: 2 }), _jsx("span", { children: children })] }));
    }
    return (_jsxs(motion.h2, { ref: ref, className: "flex items-center justify-center sm:justify-start gap-x-3 text-2xl sm:text-3xl font-semibold font-lostheading text-white theme-is-light:text-themeLight-headingText", initial: { opacity: 0, y: 20 }, animate: isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }, transition: { duration: 0.6, ease: "easeOut" }, children: [_jsx(Icon, { className: "w-7 h-7 text-purple-400 theme-is-light:text-themeLight-primary", strokeWidth: 2 }), _jsx("span", { children: children })] }));
};
export default function Faq() {
    ...
    return (_jsx(Section, { id: "faq", className: "min-h-screen pt-20 pb-20", children: _jsxs("div", { className: "max-w-7xl mx-auto px-4", children: [_jsx(Heading, { size: "xl", className: clsx("text-center sm:text-center mb-12 font-losttitle", "text-white", "theme-is-light:text-themeLight-headingText"), children: "Frequently Asked Questions" }), _jsx(SharedCard, { motion: { enableEntry: !isMotionDisabled }, className: "mb-12 text-center", textStyles: {
                        base: "font-lostbody text-lg text-lavender-200 theme-is-light:text-themeLight-text font-bold",
                    }, children: _jsxs("p", { children: ["Here you'll find answers to common questions. If you don't see your question here, feel free to reach out via", " ", _jsx("a", { href: "mailto:feedback@lemon4ik.kz", className: accentLinkStyles, children: "feedback@lemon4ik.kz" }), "."] }) }), _jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-x-8 lg:gap-x-12 gap-y-10", children: [_jsxs("div", { className: "space-y-6", children: [_jsx(CategoryHeader, { icon: ShieldCheck, children: "General & Safety" }), generalFaqData.map((item) => (_jsx(FaqItem, { item: item, isOpen: openQuestion === item.question, onClick: () => toggleFaqItem(item.question) }, item.question))), _jsxs("div", { className: "pt-4 space-y-6", children: [_jsx(CategoryHeader, { icon: Calculator, children: "Functionality" }), functionalityFaqData.map((item) => (_jsx(FaqItem, { item: item, isOpen: openQuestion === item.question, onClick: () => toggleFaqItem(item.question) }, item.question)))] })] }), _jsxs("div", { className: "space-y-6", children: [_jsx(CategoryHeader, { icon: Wrench, children: "Technical & Setup" }), technicalFaqData.map((item) => (_jsx(FaqItem, { item: item, isOpen: openQuestion === item.question, onClick: () => toggleFaqItem(item.question) }, item.question)))] })] })] }) }));
}
